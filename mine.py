...
def handle_deleted_message(message):
    if message.chat.type == 'channel':
        if message.content_type == 'delete_chat_photo':
            bot.send_message(developer_chat_id, f"Admin deleted a photo from the channel.")
        elif message.content_type == 'delete_message':
            bot.send_message(developer_chat_id, f"Admin deleted a message from the channel.")
